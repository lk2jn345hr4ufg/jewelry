...
<head>
    ...
    <title><?php echo $__env->yieldContent('title', 'JewelFind — Jewelry Business Directory'); ?></title>
    <meta name="description" content="<?php echo $__env->yieldContent('meta_description', 'Find trusted jewelry businesses near you: jewelers, watch repair, engagement rings, appraisers and more.'); ?>">
...
<header style="background:var(--ink)" class="text-white">
    <div class="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between gap-4">
        ...
        <nav class="flex items-center gap-2 sm:gap-6 text-sm">
            <a href="<?php echo e(route('home')); ?>" class="hover:text-[color:var(--champagne)] px-2 py-1">Home</a>
            <a href="<?php echo e(route('coupons.index')); ?>" class="hover:text-[color:var(--champagne)] px-2 py-1">Stores &amp; Deals</a>
            <a href="<?php echo e(route('search')); ?>" class="hidden sm:inline hover:text-[color:var(--champagne)] px-2 py-1">All businesses</a>
        </nav>
    </div>
</header>
    ...
    <div class="max-w-6xl mx-auto px-4 py-10 grid gap-8 sm:grid-cols-3 text-sm">
        ...
        <div>
            ...
            <ul class="space-y-2 text-white/70">
                <li><a href="<?php echo e(route('home')); ?>" class="hover:text-[color:var(--champagne)]">Browse by city</a></li>
                <li><a href="<?php echo e(route('coupons.index')); ?>" class="hover:text-[color:var(--champagne)]">Stores, coupons &amp; deals</a></li>
                <li><a href="<?php echo e(route('search')); ?>" class="hover:text-[color:var(--champagne)]">Search businesses</a></li>
            </ul>
        </div>
        <div>
            <p class="eyebrow mb-3">Owners</p>
            <ul class="space-y-2 text-white/70">
                <li><a href="<?php echo e(route('admin.login')); ?>" class="hover:text-[color:var(--champagne)]">Admin sign in</a></li>
            </ul>
        </div>
    </div>
    <div class="border-t border-white/10 py-4 text-center text-xs text-white/40">© <?php echo e(date('Y')); ?> JewelFind</div>
</footer>
...
document.querySelectorAll('[data-suggest]').forEach(function (input) {
    ...
    input.addEventListener('input', function () {
        ...
        timer = setTimeout(async function () {
            ...
            try {
                const res = await fetch(`<?php echo e(route('search.suggest')); ?>?q=${encodeURIComponent(q)}`, { signal: controller.signal });
...
<?php /**PATH /Users/olegmishyn/Herd/jewelry-directory/resources/views/layouts/app.blade.php ENDPATH**/ ?>