{{-- Search form with auto-suggestion. Optional: $placeholder --}}
<form action="{{ route('search') }}" method="GET" class="suggest-wrap relative w-full max-w-2xl {{ $class ?? '' }}">
    <div class="flex bg-white border border-[color:var(--line)] overflow-hidden shadow-sm">
        <input
            ...
            data-suggest
            autocomplete="off"
            placeholder="{{ $placeholder ?? 'Search jewelers, watch repair, engagement rings…' }}"
